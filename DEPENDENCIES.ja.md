...
6. [Gemfile の作成（Ruby ライブラリ管理）](#5-gemfile-の作成ruby-ライブラリ管理)
...
## 5. Gemfile の作成（Ruby ライブラリ管理）

現在のリポジトリには `Gemfile` が存在しないため、以下の手順で作成してください。
...
`Gemfile` が生成されたら、PhyloCGN が必要とする gem を追記します。
（必要な gem はスクリプト内の `require` 文を参照してください）
...
# 必要に応じて gem を追記してください
# gem "bio"
# gem "parallel"
```

gem を追記したら、インストールを実行します:
